...
enum ClaudeCostNormalizerGoldenScenarios {
    ...
    struct Projection: Codable {
        ...
        let category: String
        let costSummary: CostSummaryInfo?
        let unpricedModels: [String]?
    }

    static let periodsAccountUnpriced = ScenarioInput(
        accountEmail: "  ledger@example.test \n",
        extra: [
            ...
            "overall.rangeLabel": AnyCodable("Ignored historical range"),
            "overall.unpricedModels": array([
                AnyCodable("claude-zeta"),
                AnyCodable(7),
                AnyCodable("claude-alpha"),
            ]),
        ]
    )
    ...
    static let modelBreakdowns = ScenarioInput(
        ...
        extra: [
            ...
            "today.models": array([
                object(["model": AnyCodable("today-only")]),
            ]),
            "currentWeek.models": array([]),
            "overall.models": array([
                object([
                    "model": AnyCodable("overall-model"),
    ...
    static func normalize(_ input: ScenarioInput) -> Projection {
        ...
        let summary = UsageNormalizer.normalize(provider: FixtureClaudeProvider(), usage: usage)
        return Projection(
    ...
    private struct FixtureClaudeProvider: ProviderFetcher {
        let id = "claude"
        let displayName = "Claude Code Spend"
        let description = "Claude cost normalization fixture"
